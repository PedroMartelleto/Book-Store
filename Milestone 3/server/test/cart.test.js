...
test("an user interacting with their own cart", async () => {
    ...
    await api.addProduct(tokenAdmin, item1)
    await api.addProduct(tokenAdmin, item2)

    let productId1 = item1._id
    let productId2 = item2._id
...
test ("an user interacting with another user's cart", async () => {
    ...
    await api.addProduct(token1, item1)
    await api.addProduct(token1, item2)
...
test("an admin accessing another user's cart", async () => {
    ...
    await api.addProduct(token, item1)
    await api.addProduct(token, item2)
